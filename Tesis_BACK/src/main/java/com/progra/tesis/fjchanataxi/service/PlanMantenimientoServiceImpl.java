...
public class PlanMantenimientoServiceImpl implements PlanMantenimientoService {
    ...
    private void calcularProximosSiCorresponde(PlanMantenimiento plan, PlanDTO dto) {
        boolean proximoKmInformado = dto != null && dto.getProximoKm() != null;
        boolean proximaFechaInformada = dto != null && dto.getProximaFecha() != null;

        if (!proximoKmInformado && plan.getProximoKm() == null && plan.getFrecuenciaKm() != null) {
            obtenerUltimoKmVehiculo(plan.getVehiculo()).ifPresent(ultimoKm ->
                    plan.setProximoKm(ultimoKm.intValue() + plan.getFrecuenciaKm()));
        }
        if (!proximaFechaInformada && plan.getFrecuenciaDias() != null && plan.getProximaFecha() == null) {
            plan.setProximaFecha(LocalDate.now().plusDays(plan.getFrecuenciaDias()));
        }
    }

    private void validarPlan(PlanDTO dto) {
        if (dto.getFrecuenciaKm() != null && dto.getFrecuenciaKm() <= 0) {
            throw new ReglaNegocioException("La frecuencia en kilómetros debe ser mayor a cero");
        }
        if (dto.getFrecuenciaDias() != null && dto.getFrecuenciaDias() <= 0) {
            throw new ReglaNegocioException("La frecuencia en días debe ser mayor a cero");
        }
        if (dto.getProximoKm() != null && dto.getProximoKm() < 0) {
            throw new ReglaNegocioException("El próximo kilometraje no puede ser negativo");
        }
    }

    private java.util.Optional<Long> obtenerUltimoKmVehiculo(Vehiculo vehiculo) {
    ...
    private void sincronizarAlertasPlan(PlanMantenimiento plan) {
        if (plan == null || plan.getId() == null) {
            return;
        }
        if (!Boolean.TRUE.equals(plan.getActivo())) {
            cancelarAlertasPendientes(plan);
            return;
        }
        sincronizarAlertaFecha(plan);
    }

    private void cancelarAlertasPendientes(PlanMantenimiento plan) {
        alertaRepo.findByPlanIdAndEstado(plan.getId(), EstadoAlerta.PENDIENTE)
                .forEach(alerta -> {
                    alerta.setEstado(EstadoAlerta.CANCELADA);
    ...
    private void sincronizarAlertaFecha(PlanMantenimiento plan) {
        ...
        if (pendientes.size() > 1) {
            pendientes.subList(1, pendientes.size()).forEach(duplicada -> {
                duplicada.setEstado(EstadoAlerta.CANCELADA);
                alertaRepo.save(duplicada);
            });
        }
    }
}
