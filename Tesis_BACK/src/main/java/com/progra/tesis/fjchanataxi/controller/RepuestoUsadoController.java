...
public class RepuestoUsadoController {
    ...
    @GetMapping
    @PreAuthorize("hasAnyRole('ADMIN','TECNICO')")
    public ResponseEntity<List<RepuestoUsadoDTO>> listar(@RequestParam(name = "sinTarea", required = false) Boolean sinTarea) {
    ...
    @GetMapping("/{id}")
    @PreAuthorize("hasAnyRole('ADMIN','TECNICO')")
    public ResponseEntity<RepuestoUsadoDTO> obtener(@PathVariable("id") @Min(1) Long id) {
        return ResponseEntity.ok(repuestoService.obtener(id));
    }

    @GetMapping("/tarea/{tareaId}")
    @PreAuthorize("hasAnyRole('ADMIN','OPERADOR','TECNICO')")
    public ResponseEntity<List<RepuestoUsadoDTO>> listarPorTarea(@PathVariable("tareaId") @Min(1) Long tareaId) {
        return ResponseEntity.ok(repuestoService.listarPorTarea(tareaId));
    }

    @GetMapping("/orden/{ordenId}")
    @PreAuthorize("hasAnyRole('ADMIN','OPERADOR','TECNICO')")
    public ResponseEntity<List<RepuestoUsadoDTO>> listarPorOrden(@PathVariable("ordenId") @Min(1) Long ordenId) {
        return ResponseEntity.ok(repuestoService.listarPorOrden(ordenId));
    }

    @GetMapping("/orden/{ordenId}/total")
    @PreAuthorize("hasAnyRole('ADMIN','OPERADOR','TECNICO')")
    public ResponseEntity<BigDecimal> totalPorOrden(@PathVariable("ordenId") @Min(1) Long ordenId) {
        return ResponseEntity.ok(repuestoService.totalPorOrden(ordenId));
    }

    @PutMapping(path = "/{id}", consumes = MediaType.APPLICATION_JSON_VALUE)
    @PreAuthorize("hasAnyRole('ADMIN','TECNICO')")
    public ResponseEntity<RepuestoUsadoDTO> actualizar(@PathVariable("id") @Min(1) Long id,
