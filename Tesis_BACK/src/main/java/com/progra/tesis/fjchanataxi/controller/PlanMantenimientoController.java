...
public class PlanMantenimientoController {
    ...
    public ResponseEntity<PlanDTO> obtener(@PathVariable("id") @Min(1) Long id) {
        return ResponseEntity.ok(planService.obtener(id));
    }

    @GetMapping("/vehiculo/{vehiculoId}")
    @PreAuthorize("hasAnyRole('ADMIN','TECNICO')")
    public ResponseEntity<List<PlanDTO>> listarPorVehiculo(@PathVariable("vehiculoId") Long vehiculoId) {
        return ResponseEntity.ok(planService.listarPorVehiculo(vehiculoId));
    }

    @GetMapping("/vehiculo/{vehiculoId}/activos")
    @PreAuthorize("hasAnyRole('ADMIN','TECNICO')")
    public ResponseEntity<List<PlanDTO>> listarActivosPorVehiculo(@PathVariable("vehiculoId") Long vehiculoId) {
    ...
    public ResponseEntity<List<PlanDTO>> buscarPorNombre(@RequestParam("nombre") String nombre) {
        return ResponseEntity.ok(planService.buscarPorNombreLike(nombre));
    }

    @GetMapping("/vehiculo/{vehiculoId}/proximos-km")
    @PreAuthorize("hasAnyRole('ADMIN','OPERADOR','TECNICO')")
    public ResponseEntity<List<PlanDTO>> proximosPorKm(@PathVariable("vehiculoId") Long vehiculoId,
                                                        @RequestParam(name = "umbralKm", defaultValue = "1500") int umbralKm) {
        return ResponseEntity.ok(planService.proximosPorKm(vehiculoId, umbralKm));
    }

    @GetMapping("/vehiculo/{vehiculoId}/vencidos-km")
    @PreAuthorize("hasAnyRole('ADMIN','OPERADOR','TECNICO')")
    public ResponseEntity<List<PlanDTO>> vencidosPorKm(@PathVariable("vehiculoId") Long vehiculoId) {
        return ResponseEntity.ok(planService.vencidosPorKm(vehiculoId));
    }

    @GetMapping("/vehiculo/{vehiculoId}/proximos-fecha")
    @PreAuthorize("hasAnyRole('ADMIN','OPERADOR','TECNICO')")
    public ResponseEntity<List<PlanDTO>> proximosPorFecha(@PathVariable("vehiculoId") Long vehiculoId,
                                                           @RequestParam(name = "umbralDias", defaultValue = "30") int umbralDias) {
        return ResponseEntity.ok(planService.proximosPorFecha(vehiculoId, umbralDias));
    }

    @GetMapping("/vehiculo/{vehiculoId}/vencidos-fecha")
    @PreAuthorize("hasAnyRole('ADMIN','OPERADOR','TECNICO')")
    public ResponseEntity<List<PlanDTO>> vencidosPorFecha(@PathVariable("vehiculoId") Long vehiculoId) {
    ...
    @PreAuthorize("hasAnyRole('ADMIN','OPERADOR','TECNICO')")
    public ResponseEntity<PlanDTO> actualizar(@PathVariable("id") @Min(1) Long id, @RequestBody PlanDTO dto) {
