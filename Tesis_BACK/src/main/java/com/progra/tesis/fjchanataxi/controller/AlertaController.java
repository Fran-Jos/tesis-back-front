...
public class AlertaController {
    ...
    public ResponseEntity<AlertaDTO> cambiarEstado(@PathVariable("id") @Min(1) Long id, @RequestBody AlertaDTO dto) {
    ...
    public ResponseEntity<AlertaDTO> obtener(@PathVariable("id") @Min(1) Long id) {
        return ResponseEntity.ok(alertaService.obtener(id));
    }

    @GetMapping("/vehiculo/{vehiculoId}/pendientes")
    @PreAuthorize("hasAnyRole('ADMIN','OPERADOR','TECNICO')")
    public ResponseEntity<List<AlertaDTO>> pendientesPorVehiculo(@PathVariable("vehiculoId") Long vehiculoId) {
        return ResponseEntity.ok(alertaService.listarPendientesPorVehiculo(vehiculoId));
    }

    @GetMapping("/vehiculo/{vehiculoId}")
    @PreAuthorize("hasAnyRole('ADMIN','OPERADOR','TECNICO')")
    public ResponseEntity<List<AlertaDTO>> listarPorVehiculo(@PathVariable("vehiculoId") Long vehiculoId) {
        return ResponseEntity.ok(alertaService.listarPorVehiculo(vehiculoId));
    }

    @GetMapping("/plan/{planId}")
    @PreAuthorize("hasAnyRole('ADMIN','OPERADOR','TECNICO')")
    public ResponseEntity<List<AlertaDTO>> listarPorPlan(@PathVariable("planId") Long planId) {
        return ResponseEntity.ok(alertaService.listarPorPlan(planId));
    }

    @GetMapping("/estado/{estado}")
    @PreAuthorize("hasAnyRole('ADMIN','OPERADOR','TECNICO')")
    public ResponseEntity<List<AlertaDTO>> listarPorEstado(@PathVariable("estado") EstadoAlerta estado) {
        return ResponseEntity.ok(alertaService.listarPorEstado(estado));
    }

    @GetMapping("/tipo/{tipo}")
    @PreAuthorize("hasAnyRole('ADMIN','OPERADOR','TECNICO')")
    public ResponseEntity<List<AlertaDTO>> listarPorTipo(@PathVariable("tipo") TipoAlerta tipo) {
        return ResponseEntity.ok(alertaService.listarPorTipo(tipo));
    }

    @GetMapping("/clasificacion/{clasificacion}")
    @PreAuthorize("hasAnyRole('ADMIN','OPERADOR','TECNICO')")
    public ResponseEntity<List<AlertaDTO>> listarPorClasificacion(@PathVariable("clasificacion") ClasificacionAlerta clasificacion) {
    ...
    @GetMapping("/proximas")
    @PreAuthorize("hasAnyRole('ADMIN','OPERADOR','TECNICO')")
    public ResponseEntity<List<AlertaDTO>> proximas(@RequestParam(name = "dias", defaultValue = "7") int dias) {
        return ResponseEntity.ok(alertaService.proximasEnDias(dias));
    }

    @GetMapping("/rango-fecha")
    @PreAuthorize("hasAnyRole('ADMIN','OPERADOR','TECNICO')")
    public ResponseEntity<List<AlertaDTO>> buscarPorRangoFecha(@RequestParam("desde") LocalDate desde,
                                                               @RequestParam("hasta") LocalDate hasta,
                                                               @RequestParam(name = "estado", required = false) EstadoAlerta estado) {
        return ResponseEntity.ok(alertaService.buscarPorRangoFecha(desde, hasta, estado));
    }

    @GetMapping("/vehiculo/{vehiculoId}/rango-fecha")
    @PreAuthorize("hasAnyRole('ADMIN','OPERADOR','TECNICO')")
    public ResponseEntity<List<AlertaDTO>> buscarPorVehiculoYRango(@PathVariable("vehiculoId") Long vehiculoId,
