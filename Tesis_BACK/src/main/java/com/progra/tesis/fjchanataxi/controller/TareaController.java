...
public class TareaController {
    ...
    public ResponseEntity<List<TareaDTO>> listar(@RequestParam(name = "nombre", required = false) String nombre) {
        return ResponseEntity.ok(tareaService.listar(nombre));
    }

    @GetMapping("/selector")
    @PreAuthorize("hasAnyRole('ADMIN','TECNICO')")
    public ResponseEntity<List<TareaDTO>> listarParaSelector(@RequestParam(name = "q", required = false) String busqueda) {
        return ResponseEntity.ok(tareaService.listarParaSelector(busqueda));
    }
    ...
    @GetMapping("/{id}")
    @PreAuthorize("hasAnyRole('ADMIN','TECNICO')")
    public ResponseEntity<TareaDTO> obtener(@PathVariable("id") @Min(1) Long id) {
        return ResponseEntity.ok(tareaService.obtener(id));
    }

    @GetMapping("/orden/{ordenId}")
    @PreAuthorize("hasAnyRole('ADMIN','OPERADOR','TECNICO')")
    public ResponseEntity<List<TareaDTO>> listarPorOrden(@PathVariable("ordenId") @Min(1) Long ordenId) {
        return ResponseEntity.ok(tareaService.listarPorOrden(ordenId));
    }

    @GetMapping("/orden/{ordenId}/conteo")
    @PreAuthorize("hasAnyRole('ADMIN','OPERADOR','TECNICO')")
    public ResponseEntity<Long> contarPorOrden(@PathVariable("ordenId") @Min(1) Long ordenId) {
        return ResponseEntity.ok(tareaService.contarPorOrden(ordenId));
    }

    @GetMapping("/tecnico/{usuarioId}")
    @PreAuthorize("hasAnyRole('ADMIN','OPERADOR','TECNICO')")
    public ResponseEntity<List<TareaDTO>> listarPorTecnico(@PathVariable("usuarioId") @Min(1) Long usuarioId,
                                                           @RequestParam(name = "estado", required = false) EstadoTarea estado) {
        return ResponseEntity.ok(tareaService.listarPorTecnico(usuarioId, estado));
    }

    @PutMapping(path = "/{id}", consumes = MediaType.APPLICATION_JSON_VALUE)
    @PreAuthorize("hasAnyRole('ADMIN','TECNICO')")
    public ResponseEntity<TareaDTO> actualizar(@PathVariable("id") @Min(1) Long id, @RequestBody TareaDTO dto) {
        return ResponseEntity.ok(tareaService.actualizar(id, dto));
    }

    @PatchMapping("/{id}/estado")
    @PreAuthorize("hasAnyRole('ADMIN','TECNICO')")
    public ResponseEntity<TareaDTO> actualizarEstado(@PathVariable("id") @Min(1) Long id,
    ...
    public ResponseEntity<TareaDTO> actualizarAsignacion(@PathVariable("id") @Min(1) Long id,
