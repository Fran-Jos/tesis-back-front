...
const MantenimientoDetalladoReportPage = () => {
  ...
  const handleBuscar = async () => {
    ...
    try {
      ...
      setError(resolveErrorMessage(err, "No se pudo generar el reporte"));
      setReportes([]);
    } finally {
      setLoading(false);
    }
  ...
  const handleBuscarPorCodigo = async () => {
    ...
    try {
      ...
      setError(resolveErrorMessage(err, "No se encontró una orden con ese código"));
      setReportes([]);
    } finally {
      setLoading(false);
    }
  ...
  const handleDescargarPdf = async () => {
    ...
    try {
      ...
      setDownloadError(resolveErrorMessage(err, "No se pudo descargar el PDF"));
    } finally {
        ...
        {reportes.map((orden) => (
          <article key={orden.ordenId} className="space-y-4 rounded-3xl border border-slate-200 bg-white p-6 shadow-xl">
            ...
            <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
              ...
              <div>
                <p className="text-xs font-semibold uppercase tracking-widest text-slate-400">Kilometraje</p>
                <p className="text-sm text-slate-600">
                  {orden.kilometraje ? `${orden.kilometraje.toLocaleString("es-EC")} km` : "No registrado"}
                </p>
              </div>
              <div>
                <p className="text-xs font-semibold uppercase tracking-widest text-slate-400">Cerrado por</p>
                <p className="text-sm text-slate-600">{orden.cerradoPorNombre ?? "No registrado"}</p>
              </div>
              <div>
                <p className="text-xs font-semibold uppercase tracking-widest text-slate-400">Detalle</p>
                <p className="text-sm text-slate-600">{orden.detalle ?? "No registrado"}</p>
              </div>
              <div>
                        ...
                        {tarea.repuestos.length === 0 ? (
                          ...
                          <ul className="space-y-2">
                            {tarea.repuestos.map((repuesto) => (
                              <li
                                key={repuesto.id}
                                className="flex flex-col gap-1 rounded-xl border border-slate-200 px-3 py-2 text-sm text-slate-600 md:flex-row md:items-center md:justify-between"
                              >
                                <span className="font-medium text-slate-800">{repuesto.descripcion}</span>
                                <span>
