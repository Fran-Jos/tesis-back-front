...
const EntityListPage = ({ config }: EntityListPageProps) => {
  ...
  useEffect(() => {
    ...
    const loadFilterOptions = async () => {
      ...
      // Limpiamos opciones previas al cambiar de entidad
      setFilterOptions({});
          ...
          config.list.filters
            ...
            .map(async (filter) => {
              ...
              try {
                ...
                console.error(`Error cargando opciones para filtro ${filter.name}:`, err);
              }
            }),
        );
      } catch (err) {
        console.error("Error general cargando opciones de filtros:", err);
      }
    };
  ...
  }, [config.key, config.list.filters]);
                  ...
                  <tr key={item.id ?? crypto.randomUUID()} className="transition hover:bg-slate-50/70">
                    {config.list.columns.map((column) => {
                      ...
                      if ((value === null || value === undefined || value === "") && authUser) {
                        ...
                        if (column.field === "creadoPorNombre") {
                          idValue = item.creadoPorId || item.creado_por_id;
                        } else if (column.field === "usuarioNombre") {
                          idValue = item.usuarioId || item.usuario_id;
                        } else if (column.field === "responsableNombre") {
                          idValue = item.responsableId || item.responsable_id;
                        } else if (column.field === "asignadoANombre") {
                    ...
                    <td className="px-4 py-3">
                      <div className="flex flex-wrap gap-2">
                        ...
                        {config.key === "tareas" && item.id ? (
                          <Link
                            to={`/app/repuestos-usados/nuevo?tareaId=${encodeURIComponent(String(item.id))}`}
                            className="rounded-xl border border-emerald-100 bg-emerald-50 px-3 py-1 text-xs font-semibold text-emerald-700 transition hover:bg-emerald-100"
                          >
                            Agregar repuesto
                          </Link>
                        ) : null}
